		...
		if ($content !== false) {
			...
			foreach ($items as $item) {
				...
				if (isset($matches[1])) {
					...
					if (count($matches) == 2) {
						echo '<li><a href="#" onClick="LaunchHelp(\''.IEM::enableInfoTipsGet().'\',\'http://anonym.to/?' . $matches[1] . '\');" title="' . htmlspecialchars($title, ENT_QUOTES, SENDSTUDIO_CHARSET) . '">' . $this->TruncateInMiddle($title) . '</a></li>';
					} else {
