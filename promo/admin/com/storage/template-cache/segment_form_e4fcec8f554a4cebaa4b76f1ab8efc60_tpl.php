...
<script>
	var PAGE = {
		INIT_VALUES:					'<?php if(isset($GLOBALS['InitialValues'])) print $GLOBALS['InitialValues']; ?>',
		...
		_cacheRules:					{	'email':		{	'type':			'text',
																'value':		'email',
																'text':			'<?php print GetLang('Email'); ?>',
																'display':		true,
																'operatortype':	'text'},
											'format':		{	'type':			'dropdown',
																'value':		'format',
																'text':			'<?php print GetLang('Format'); ?>',
																'display':		true,
																'operatortype':	'dropdown'},
											'confirmation':	{	'type':			'dropdown',
																'value':		'confirmation',
																'text':			'<?php print GetLang('ConfirmedStatus'); ?>',
																'display':		true,
																'operatortype':	'dropdown'},
											'status':		{	'type':			'dropdown',
																'value':		'status',
																'text':			'<?php print GetLang('FilterStatus'); ?>',
																'display':		true,
																'operatortype':	'dropdown'},
											'subscribe':	{	'type':			'date',
																'value':		'subscribe',
																'text':			'<?php print GetLang('FilterByDate'); ?>',
																'display':		true,
																'operatortype':	'date'},
											'link':			{	'type':			'dropdown',
																'value':		'link',
																'text':			'<?php print GetLang('ClickedOnLink'); ?>',
																'display':		true,
																'operatortype':	'link'},
											'campaign':		{	'type':			'dropdown',
																'value':		'campaign',
																'text':			'<?php print GetLang('OpenedNewsletter'); ?>',
																'display':		true,
																'operatortype':	'campaign'}},
		_cacheRulesCustomFields:		[	{'value': 'isempty', 'text': '<?php print GetLang('SegmentFormOperator_common_customfields_isempty'); ?>'},
											{'value': 'isnotempty', 'text': '<?php print GetLang('SegmentFormOperator_common_customfields_isnotempty'); ?>'}],
		_cacheRuleOperations:			{	'text':		[	{'value': 'equalto', 'text': '<?php print GetLang('SegmentFormOperator_text_equalto'); ?>'},
															{'value': 'notequalto', 'text': '<?php print GetLang('SegmentFormOperator_text_notequalto'); ?>'},
															{'value': 'like', 'text': '<?php print GetLang('SegmentFormOperator_text_like'); ?>'},
															{'value': 'notlike', 'text': '<?php print GetLang('SegmentFormOperator_text_notlike'); ?>'}],
											'dropdown':	[	{'value': 'equalto', 'text': '<?php print GetLang('SegmentFormOperator_dropdown_equalto'); ?>'},
															{'value': 'notequalto', 'text': '<?php print GetLang('SegmentFormOperator_dropdown_notequalto'); ?>'}],
											'number':	[	{'value': 'equalto', 'text': '<?php print GetLang('SegmentFormOperator_number_equalto'); ?>'},
															{'value': 'notequalto', 'text': '<?php print GetLang('SegmentFormOperator_number_notequalto'); ?>'},
															{'value': 'greaterthan', 'text': '<?php print GetLang('SegmentFormOperator_number_greaterthan'); ?>'},
															{'value': 'lessthan', 'text': '<?php print GetLang('SegmentFormOperator_number_lessthan'); ?>'},
															{'value': 'between', 'text': '<?php print GetLang('SegmentFormOperator_number_between'); ?>'}],
											'multiple':	[	{'value': 'equalto', 'text': '<?php print GetLang('SegmentFormOperator_multiple_equalto'); ?>'},
															{'value': 'notequalto', 'text': '<?php print GetLang('SegmentFormOperator_multiple_notequalto'); ?>'}],
											'date': 	[	{'value': 'equalto', 'text': '<?php print GetLang('SegmentFormOperator_date_equalto'); ?>'},
															{'value': 'notequalto', 'text': '<?php print GetLang('SegmentFormOperator_date_notequalto'); ?>'},
															{'value': 'greaterthan', 'text': '<?php print GetLang('SegmentFormOperator_date_greaterthan'); ?>'},
															{'value': 'lessthan', 'text': '<?php print GetLang('SegmentFormOperator_date_lessthan'); ?>'},
															{'value': 'between', 'text': '<?php print GetLang('SegmentFormOperator_date_between'); ?>'}],
											'link':		[	{'value': 'equalto', 'text': '<?php print GetLang('SegmentFormOperator_link_equalto'); ?>'},
															{'value': 'notequalto', 'text': '<?php print GetLang('SegmentFormOperator_link_notequalto'); ?>'}],
											'campaign':	[	{'value': 'equalto', 'text': '<?php print GetLang('SegmentFormOperator_campaign_equalto'); ?>'},
															{'value': 'notequalto', 'text': '<?php print GetLang('SegmentFormOperator_campaign_notequalto'); ?>'}]},
		_cacheRuleValues:				{	'format':		[	{'value': 'h', 'text': '<?php print GetLang('Format_HTML'); ?>'},
																{'value': 't', 'text': '<?php print GetLang('Format_Text'); ?>'}],
											'confirmation':	[	{'value': '1', 'text': '<?php print GetLang('Confirmed'); ?>', 'selected': true},
																{'value': '0', 'text': '<?php print GetLang('Unconfirmed'); ?>'}],
											'status':		[	{'value': 'a', 'text': '<?php print GetLang('Active'); ?>'},
																{'value': 'b', 'text': '<?php print GetLang('Bounced'); ?>'},
																{'value': 'u', 'text': '<?php print GetLang('Unsubscribed'); ?>'}]},
		_cacheMailingList:				{},
		...
		init:							function() {
			...
			$('.cancelButton').click(function() {
				if(confirm('<?php print GetLang('SegmentFormAlertCancel'); ?>')) Application.Util.submitGet('index.php', {Page:'Segment'});
			});
			...
			if(this.INIT_VALUES != '' && this.INIT_VALUES != '{}') {
				try {
					...
					alert('<?php print GetLang('SegmentFormAlertInitializingValues'); ?>');
				}
			} else {
		...
		submit:							function() {
			...
			if(data.SegmentName == '') {
				alert('<?php print GetLang('SegmentFormAlertSpecifySegmentName'); ?>');
			...
			if(data.Lists.length == 0) {
				alert('<?php print GetLang('SegmentFormAlertAtLeastOneList'); ?>');
				this._disableSubmit = false;
				return;
			}

			if(data.Rules.length == 0) {
				alert('<?php print GetLang('SegmentFormAlertAtLeastOneRule'); ?>');
				this._disableSubmit = false;
				return;
			}
		...
		ruleAddMatchType:				function() {
			var matchType = '&nbsp;' + ($('#matchType_All, #matchType_Or', document.frmSegmentEditor).filter(':checked').val() == 'or'? '<?php print GetLang('OR'); ?>' : '<?php print GetLang('AND'); ?>') + '&nbsp;';
		...
		_addRuleSelectOptions:			function(elm, data) {
			...
			$(	'<optgroup label="<?php echo GetLang('SegmentCustomField_Basic'); ?>">' + selectOptions.basic.join('') + '</optgroup>'
				+ (selectOptions.customfields.length == 0? '' : '<optgroup label="<?php echo GetLang('SegmentCustomField_CustomField'); ?>">' + selectOptions.customfields.join('') + '</optgroup>')).appendTo(elm);
		...
		_listRequestListData_CB:		function(response) {
			...
			catch(e) { alert('<?php print GetLang('SegmentFormAlertErrorRequestingData'); ?>'); }
			...
			if(ruleName.selectedIndex >= 0) {
				...
				switch(ruleType) {
					...
					case 'checkbox':
						...
						var tempOuputPlaceholder = $('<input id="RuleValue_Placeholder_' + $(row).attr('rowIndex') + '" type="text" readonly="readonly" value="'+ (data? data.join(', ') : '<?php print GetLang('SegmentFormCheckbox_SelectInstruction'); ?>') +'" title="<?php print GetLang('SegmentFormCheckbox_SelectTooltip'); ?>" />');
						...
						tempOuputPlaceholder.click(function() {
							...
							tb_show('<?php print GetLang('SegmentFormCheckbox_SelectWindowTitle'); ?>', '#TB_inline?height=300&width=400&inlineId=RuleValue_MultipleSelectorPlaceholder_'+id, false);
						});
		...
		_appendDateBetween:				function(row) {
			var temp = $('<span>&nbsp;<?php print GetLang('AND'); ?>&nbsp;<input type="text" readonly="readonly" class="RuleRowUserInput" name="RuleValue[' + $(row).attr('rowIndex') + '][]" /></span>');
			temp.appendTo($('.RuleRowRuleValues', row).get(0));
			$(temp.children().get(0)).datepicker();
		},
		_appendNumberBetween:			function(row) {
			var temp = $('<span>&nbsp;<?php print GetLang('AND'); ?>&nbsp;<input type="text" class="RuleRowUserInput" name="RuleValue[' + $(row).attr('rowIndex') + '][]" /></span>');
			temp.appendTo($('.RuleRowRuleValues', row).get(0));
			this._applyNumberInputboxBehaviour(temp.get(0));
		}
	};
...
<form name="frmSegmentEditor" id="frmSegmentEditor" method="post" action="index.php">
	<input type="hidden" name="SegmentID" value="<?php if(isset($GLOBALS['SegmentID'])) print $GLOBALS['SegmentID']; ?>" />
	<table cellspacing="0" cellpadding="3" width="100%" align="center">
		...
		<tr>
			<td>
				<input class="FormButton submitButton" type="button" value="<?php print GetLang('Save'); ?>">
				<input class="FormButton cancelButton" type="button" value="<?php print GetLang('Cancel'); ?>" />
				<br />&nbsp;
				<table border="0" cellspacing="0" cellpadding="2" width="100%" class="Panel">
					<tr>
						<td colspan="2" class="Heading2">
							&nbsp;&nbsp;<?php print GetLang('SegmentFormHeaderDetails'); ?>
						</td>
					</tr>
					<tr>
			...
			$tpl->TemplateData = $tmpTplData; ?>
							<?php print GetLang('SegmentName'); ?>:&nbsp;
						</td>
						<td width="90%">
							<input type="text" name="SegmentName" class="Field250 form_text" style="width:445px" value="<?php if(isset($GLOBALS['SegmentName'])) print $GLOBALS['SegmentName']; ?>" /><span class="HelpToolTip"><img src="images/help.gif" alt="?" width="24" height="16" border="0" /><span class="HelpToolTip_Title" style="display:none;"><?php print stripslashes(GetLang('SegmentName')); ?></span><span class="HelpToolTip_Contents" style="display:none;"><?php print stripslashes(GetLang('HLP_SegmentName')); ?></span></span>
						</td>
					</tr>
					<tr>
						<td class="FieldLabel">
			...
			$tpl->SetTemplate($tmpTplFile);
			$tpl->TemplateData = $tmpTplData; ?>
							<?php print GetLang('SegmentFormFieldMailingList'); ?>:&nbsp;
						</td>
						<td>
							<select id="SelectList" name="SelectList[]" multiple="multiple" class="ISSelectReplacement ISSelectSearch" style="<?php if(isset($GLOBALS['SelectListStyle'])) print $GLOBALS['SelectListStyle']; ?>">
								<?php if(isset($GLOBALS['SelectListHTML'])) print $GLOBALS['SelectListHTML']; ?>
							</select>&nbsp;<span class="HelpToolTip"><img src="images/help.gif" alt="?" width="24" height="16" border="0" /><span class="HelpToolTip_Title" style="display:none;"><?php print stripslashes(GetLang('SegmentFormFieldMailingList')); ?></span><span class="HelpToolTip_Contents" style="display:none;"><?php print stripslashes(GetLang('HLP_SegmentFormFieldMailingList')); ?></span></span>
						</td>
					</tr>
					<tr>
						<td class="FieldLabel">
			...
			$tpl->SetTemplate($tmpTplFile);
			$tpl->TemplateData = $tmpTplData; ?>
							<?php print GetLang('SegmentFormFieldMatchType'); ?>:&nbsp;
						</td>
						<td>
							<input type="radio" id="matchType_All" name="MatchType" value="and"<?php if(isset($GLOBALS['MatchType_AND'])) print $GLOBALS['MatchType_AND']; ?>/><label for="matchType_All"><?php print GetLang('SegmentFormMatchAllRule'); ?></label>
							&nbsp;<span class="HelpToolTip"><img src="images/help.gif" alt="?" width="24" height="16" border="0" /><span class="HelpToolTip_Title" style="display:none;"><?php print stripslashes(GetLang('SegmentFormFieldMatchType')); ?></span><span class="HelpToolTip_Contents" style="display:none;"><?php print stripslashes(GetLang('HLP_SegmentFormFieldMatchType')); ?></span></span>
							<br />
							<input type="radio" id="matchType_Or" name="MatchType" value="or"<?php if(isset($GLOBALS['MatchType_OR'])) print $GLOBALS['MatchType_OR']; ?>/><label for="matchType_Or"><?php print GetLang('SegmentFormMatchAnyRule'); ?></label>
						</td>
					</tr>
					<tr><td class="EmptyRow" colspan="2">&nbsp;</td></tr>
					<tr>
						<td colspan="2" class="Heading2">
							&nbsp;&nbsp;<?php print GetLang('SegmentFormHeaderRules'); ?>
						</td>
					</tr>
					<tr>
			...
			$tpl->SetTemplate($tmpTplFile);
			$tpl->TemplateData = $tmpTplData; ?>
							<?php print GetLang('SegmentFormRuleDescription'); ?>:
						</td>
